...
Как и прежде, финальной частью данного проекта является подведение итогов и обсуждение полученных результатов. В моем случае, это возможность проведения анализа над допущенными ошибками в целях предотвращения их при выполнении последующих проектов. Ввиду недостаточно точного имплементирования of the random walk algorithm, а также погрешностей при подсчетах путем использования SimRank algorithm, полученные результаты не могут с достоверностью оражать реальную картину того, в каком ключе протекает атака внутри системы. In my case, the algorithm is imprecise and can sometimes be confusing when analyzing the generated data. Наличие ложнонегативных и ложнопозитивных результатов также негативно сказываются на точности написанного алгоритма, что, несомненно, указывает на необходимость дальнейшней качественной доработки основных его компонентов, включая логику и общую структуру. Касательно имещихся на данный момент результатов, можно сделать вывод о том, что в процентном соотношении лишь 22-25% атак на систему является безуспешными, что, безусловно, связано с погрешностями в действующем алгоритме. When writing the algorithm, due to the lack of a clear understanding of how exactly to work with this kind of data, and due to lack of experience, I did not take into account a number of important factors that play a key role in determining the percentage of similarity between nodes and edges of both graphs. Таким образом, принимая во внимание все допущенные погрешности и неточности при выполнении проекта, я делаю вывод о том, что использование данного алгоритма недопустимо при оценке реальной угрозы атаки на системы компаний. The implemented algorithm still needs careful refinement and additional improvements in order to increase the accuracy of calculations.

Подводя итог, я в очередной раз I would like to thank you both, Dr. Mark and Utsav, for the chance to get a feel for a real example of what real programming is like that we will encounter in a real work environment. Порой осознание того, какое огромное количество часов я инвестирую в написание проекта, который в конце концов не позволяет достичь требуемую функциональность, создает иллюзию того, что я являюсь одним из героев фильма ужасов, скажем, Молчания Ягнят, где в роли "ягненка" на заклание выступаю я. И все же, невзирая на все трудности, я четко вижу насколько выросли мои навыки во всех аспектах с первого дня на данном курсе. Полученный опыт, несомненно, является самой ценной наградой.

Every project we do in this course makes me realize the importance of planning the design of a future project at its earliest stages, as well as the need to ask questions even if there are even the slightest gaps in understanding what you have to work with. A lot of mistakes made when writing the project in its early stages significantly complicated the work process later, which led to the need to rewrite the code over and over again, which, in turn, gave rise to bugs in other areas of the program. Finally, I would like to note once again that the opportunity to work on tasks of this level is invaluable, and subsequently I will work on mistakes in order to prevent them when writing upcoming projects.
